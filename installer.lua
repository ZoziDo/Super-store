-- Reactor Control + Shop Installer (для Super-store)
-- Author: Твой ник
-- ProgV 1.2 – адаптированный под структуру GitHub

------------------------------------ config ------------------------------------
local REPOSITORY  = "https://raw.githubusercontent.com/ZoziDo/Super-store/refs/heads/main/"
...
local filesToDownload = {
  ...
  {url = REPOSITORY.."lib/OCIF.lua",               path = "/lib/OCIF.lua"},
  {url = REPOSITORY.."home/nr_v1.1.lua",           path = "/home/main.lua"},
  -- если добавишь картинки позже – раскомментируй строки ниже
...
local appTitle = "Reactor Control + Shop – Installer"
...
local function drawChrome()
  ...
  text(X+W-20, Y, "[by P1KaChU337 / shop]", COL_DIM)
...
local function ensureDirs()
  if not fs.exists("/lib") then fs.makeDirectory("/lib") end
  if not fs.exists("/home") then fs.makeDirectory("/home") end
  -- подпапка для картинок – если понадобится
...
local function install()
  ...
  text(X+2, Y+H-2, "Reactor Control + Shop | Installer", COL_DIM)
